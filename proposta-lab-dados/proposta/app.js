/* =====================================================================
   NEXIFY × DAILUS · LABORATÓRIO DE DADOS — deck loader + interações
   - Mesmo motor de apresentação do ../../proposta-plataforma-influencers/.
   - Cada slide é um arquivo em ./slides/*.html (um slide por arquivo).
   - Para adicionar/remover/reordenar: edite a lista SLIDES abaixo.
   - Requer servir via HTTP (usa fetch): `npm run lab`.
...
(function(){
  ...
  var SLIDES = [
    ...
    '20-certificacao',
    '21-perguntas',
    '22-perg-objetivo-perfil',
    '23-perg-problemas-dados',
    '24-perg-entregas-governanca',
    '25-perg-investimento-carreira',
    '26-mapa',
    '27-cta',
    '28-footer'
  ];
  ...
  /* ---------------------------------------------------------------
     ...
     pelo slide-mapa (26) ou pelas miniaturas. Dentro de um ramo a
     ...
     A trilha principal fica com 18 slides e converge para um pedido só:
     aprovar a Fase 0. Os ramos existem para responder a pergunta que a
     Dailus fizer — o catálogo de ativos, o conteúdo das fases 2 a 4 e as
     50 perguntas de fechamento não precisam ser apresentados em ordem.
     --------------------------------------------------------------- */
  var HUB = '26-mapa';
  var DEEP = {
    '05-ativos-fundacao'          : 'os ativos',
    '06-ativos-clientes'          : 'os ativos',
    '07-ativos-ia'                : 'os ativos',
    '12-fase-2'                   : 'a trilha',
    '13-fase-3'                   : 'a trilha',
    '14-fase-4'                   : 'a trilha',
    '22-perg-objetivo-perfil'     : 'as 50 perguntas',
    '23-perg-problemas-dados'     : 'as 50 perguntas',
    '24-perg-entregas-governanca' : 'as 50 perguntas',
    '25-perg-investimento-carreira': 'as 50 perguntas'
  };
  ...
  function loadSlides(){
    return Promise.all(SLIDES.map(function(name){
      return fetch('slides/' + name + '.html', {cache:'no-cache'})
        ...
        .catch(function(err){
          return {name:name, html:'<div class="slide-error">Falha ao carregar slide <b>'+name+'</b> ('+err.message+'). Sirva a pasta via HTTP: <span class="mono">npm run lab</span></div>'};
        });
    })).then(function(parts){
  ...
  function initSlideshow(revealAll){
    ...
    var NAV_KEY='lab-dados-shownav-w';
